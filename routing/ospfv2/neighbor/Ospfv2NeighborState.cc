...
void NeighborState::changeState(Neighbor *neighbor, NeighborState *newState, NeighborState *currentState)
{
    ...
    if (newState->getState() == Neighbor::FULL_STATE) {
        if (simTime() > getSimulation()->getWarmupPeriod() && PRINT_FULL_DURATION) {
            std::cout << "at " << simTime() << ", router: " <<
                    neighbor->getInterface()->getArea()->getRouter()->getRouterID() <<
                    ", neighbor: " << neighbor->getNeighborID() <<
                    " FULL, duration: " << simTime() - neighbor->getNeighborInitTime() << std::endl;
        }
    }
