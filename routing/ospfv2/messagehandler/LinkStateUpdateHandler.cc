...
void LinkStateUpdateHandler::processPacket(Packet *packet, Ospfv2Interface *intf, Neighbor *neighbor)
{
    /*
     * @sqsq
     * current_ttl: ttl extracted from received packet
     * current_time: if < 60, then use original OSPF; else, use our local LS
     * TODO: how to distinguish
     */
    int current_ttl = packet->getTag<HopLimitInd>()->getHopLimit();
    int next_ttl = current_ttl;
    if (sqsqCheckSimTime()) {
        next_ttl--;
    }
    ...
    if (neighbor->getState() >= Neighbor::EXCHANGE_STATE) {
        ...
        for (unsigned int i = 0; i < lsUpdatePacket->getOspfLSAsArraySize(); i++) {
            ...
            if ((ackFlags.noLSAInstanceInDatabase) || (ackFlags.lsaIsNewer)) {  // this entire "if" is 13(5)
                 ...
                 * @sqsq
                 */
                ackFlags.floodedBackOut = router->floodLSA(currentLSA, areaID, current_ttl, intf, neighbor);  // section 13(5)(b)
                ...
                if (!ackFlags.noLSAInstanceInDatabase) {
                    ...
                    router->removeFromAllRetransmissionLists(lsaKey);  // section 13(5)(c) remove the current LSDB copy from all neighbors' retransmission lists
                }
                shouldRebuildRoutingTable |= router->installLSA(currentLSA, areaID);  // section 13(5)(d)

                EV_INFO << "    (update installed)\n";

                acknowledgeLSA(currentLSA->getHeader(), intf, ackFlags, lsUpdatePacket->getRouterID()); // 13(5)(e)

                if ((currentLSA->getHeader().getAdvertisingRouter() == router->getRouterID()) ||  // 13(5)(f)   see section 13.4  TODO: read
                    ((lsaType == NETWORKLSA_TYPE) &&
                     (router->isLocalAddress(currentLSA->getHeader().getLinkStateID()))))
                {
                    if (ackFlags.noLSAInstanceInDatabase) {
                         ...
                         * @sqsq
                         */
                        router->floodLSA(lsaCopy, areaID, current_ttl);
                    }
                    else {
                        if (ackFlags.lsaIsNewer) {
                            ...
                            if (sequenceNumber == MAX_SEQUENCE_NUMBER) {
                                 ...
                                 * @sqsq
                                 */
                                router->floodLSA(lsaInDatabase, areaID, current_ttl);
                            }
                            else {
            ...
            if (neighbor->isLSAOnRequestList(lsaKey)) {  // 13(6)
                neighbor->processEvent(Neighbor::BAD_LINK_STATE_REQUEST);
                break;
            }
            if (ackFlags.lsaIsDuplicate) {  // 13(7)
            ...
            // 13(8) the database copy is more recent
